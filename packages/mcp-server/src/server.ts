...
export function createServer(manager = GatewayManager.fromEnvironment()): McpServer {
  ...
  server.registerTool('dsh_start_or_connect', {
    description: 'Connect to an independently owned DSH Host and create or reconnect a session. Creating requires the target project absolute cwd and fixes one supervision-compatible DSH agent preset for the durable session: standard (default) or code (PTC). Reviewed supervision currently requires Standard/native tools; PTC supports delegated supervision only. Reconnecting preserves both cwd and preset and rejects a conflicting requested value. Minimal cannot guarantee the strict read-only boundary; Creator has Host-runtime authoring authority, so neither is admitted for supervised project work. MCP shutdown never stops the Host.',
  ...
  server.registerTool('dsh_task', {
    ...
    inputSchema: z.object({
      ...
      supervisionMode: z.enum(['delegated', 'reviewed']).optional()
        .describe('delegated for light supervision; reviewed for immediate reported-risk review and independent terminal verification.'),
      reviewWatchPaths: z.array(z.string().min(1).max(256)).max(16).optional()
        .describe('Workspace-relative interface or critical file/path prefixes selected by Codex for independent review; no extra write authority.'),
  ...
  server.registerTool('dsh_answer_question', {
    description: 'Answer the current bounded DSH question batch as one response. For a supervisor_review engineering checkpoint, Codex may approve a sound proposal within the existing task authority or return specific revision feedback; scope expansion, unresolved material architecture and sensitive/external effects retain the human decision boundary. A live review resumes the same run. Generic steer is not approval. rpcId must match the current pending interaction; stale replies are rejected. When the observation says answerInWeb=true, use DSH Web for the full oversized batch instead of answering its truncated preview here.',
    inputSchema: z.object({
