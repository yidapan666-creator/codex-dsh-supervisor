...
|---|---|---|---|
| DeepSeek Harness (public fork) | `a36bb20300a905e849554451cbc14d02735ed8f6` on `https://github.com/yidapan666-creator/deepseek-harness.git` (branch `codex/mcp-network-client`), fetched by SHA via `pnpm bootstrap` | MIT | Direct public API reuse via the linked network client; no DSH source is copied into this repository. The fork commit is the pinned compatibility contract, not an upstream merge. |
