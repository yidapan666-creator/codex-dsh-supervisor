...
|---|---|---|---|
| DeepSeek Harness (public fork) | `68dd149a1834496ced7308de5a7084328855f13e` on `https://github.com/yidapan666-creator/deepseek-harness.git` (branch `codex/mcp-network-client`), fetched by SHA via `pnpm bootstrap` | MIT | Direct public API reuse via the linked network client, generic durable prompt activation, and optional bearer authentication for HTTP/WebSocket carriers. The Host admission projection contains a small TypeScript adaptation of DSH's durable Inbox splice replay so cold sessions use the same pending-work semantics. The fork commit is the pinned compatibility contract, not an upstream merge. |
