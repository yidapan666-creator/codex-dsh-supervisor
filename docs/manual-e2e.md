...
Expected: Host/session survive MCP restart; replayed interactions retain a stable `rpcId` and stale replies are rejected; timeout and worker state are separate; ordinary mux churn does not cause one history request per event; activity coverage and runtime verification evidence are explicit; only the valid handoff plus turn end completes; artifact escape is rejected; the second shared-tree writer is rejected until the first root has ended and its descendants/follow-up queue are quiet; the Host stops only via its explicit stop path.
