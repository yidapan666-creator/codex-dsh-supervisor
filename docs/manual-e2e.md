...
5. Restart only MCP and call `dsh_start_or_connect` with the existing session id and no explicit Host URL. With multiple configured Hosts, confirm it discovers the non-default Host when appropriate and returns the same `hostInstanceId` and transcript — the Host outlived the MCP restart.
