...
- Handoff summaries are capped at 2048 characters. An over-limit `supervisor_handoff` fails with an actionable error telling the worker to write the detailed report under `.dsh-handoff/<taskId>/` inside the session cwd and pass its relative path in `artifacts`; the tool never auto-writes handoff data, and the `.dsh-handoff` directory is gitignored so reports never enter the repository.
