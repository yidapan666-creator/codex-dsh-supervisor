...
Every new schema-v2 run pins `supervisionMode`. `delegated` uses bounded structured terminal evidence. `reviewed` requires Codex to inspect the actual scoped changes and independently rerun relevant verification before presenting the result as accepted. Writer runs default to `reviewed`, read-only runs default to `delegated`, and callers may explicitly override either default. Recovery continuations preserve the parent mode.
...
Every valid run observation echoes the pinned supervision mode. In `reviewed` mode, an accepted worker-reported `high` or `critical` `riskLevel` is an immediate supervisor boundary even without a separate decision request. The runtime does not infer semantic risk. A reviewed `COMPLETED` observation carries `decision.action=REVIEW_TERMINAL`; protocol completion remains valid, but supervisor acceptance is still pending independent review. Worker verification `acceptanceStatus` is not a Codex review receipt.

Risk and decision facts are evaluated independently: a cadence or pre-authorized decision cannot downgrade reported high/critical risk, and a risk cannot downgrade a human-directed decision. Unhandled reviewed risks are folded from all accepted progress in the current run after the latest durable `user/message` guidance; ordinary milestones and observation cursors do not consume them. `pendingRisks` preserves their count and up to four bounded previews with original boundary sequences, marking additional entries as truncated. This is a notification ledger, not proof that guidance resolved the underlying risk. Host failures, interactions, budget stops, interrupted recovery and completed handoffs keep their protocol decisions and carry the pending risk context alongside them; a risk never replaces the lifecycle boundary.
...
- Writer execution additionally requires a Git worktree. Inside the same Host admission critical section, the runtime fsyncs an authoritative HEAD and fingerprints every pre-existing dirty path before the provider can run. Caller baseline prose is not an authority. Writer `allowedScope` entries are workspace-relative path prefixes; absent scope means the complete session cwd, while absolute/traversing scope is rejected. A `completed` handoff is rejected if the recomputed task-era committed or uncommitted path set escapes those prefixes. Rename detection is disabled for path enumeration so both the deleted source and added destination are checked, and regular-file fingerprints include permission bits. Git-ignored files are observed independently with ctime-backed metadata fingerprints; ignore rules never grant write permission. The only ignored-scan exceptions are the explicitly configured Host runtime-state tree and `.dsh-handoff`, whose contents use their own bounded, symlink-rejecting scanner. Unchanged pre-existing dirty files are not attributed to the worker; further edits or reverts of those files are.
...
- The anti-stuck mechanism counts exact worker-reported `failureSignature` values since the latest durable task packet and concludes the turn when the configured recovery budget is exhausted (two reports by default). Semantic equivalence remains the worker's judgment. If a Host interruption follows an exhausted report, the interruption remains authoritative: wait/recover return `FAILED` plus `CONTINUATION_REQUIRED` and a Host-derived capsule, while `reportedFailureBudget.exhausted` is attached as a fact instead of replacing recovery with an escalation-only state.
...
cache-read, cache-write, and output buckets without a model call. It also
reports an explicit state: `ACTIVE` while below the limit, `EXHAUSTED` when the
authoritative total equals the limit, `OVERSHOT` with `overshootTokens` when a
settled provider response crosses it, or `REQUEST_REJECTED` when the next input
was refused before a provider call. The
...
running. While the MCP process remains alive, `ConnectionController` reconnects
its DSH HTTP/WebSocket client automatically. If the stdio MCP process itself
exits, that parent-owned stdio transport is permanently closed; Codex (or the
calling MCP client) must start a fresh MCP process, which then uses `dsh_runs`
and `dsh_recover` to reattach. A server cannot resurrect a closed transport in
its already-dead process, and this boundary must not be presented as same-turn
transparent restart. A Host-process crash is different: DSH persistence closes the orphaned
...
In reviewed mode, Codex records technical direction and decision boundaries in the existing task context, constraints and executionBrief.integration. Root requests `supervisor_review` after investigation yields a nontrivial unapproved approach, before changing a public/data/persistence/recovery/authority contract, when evidence invalidates an approved plan, or when a repair must expand into broader refactoring. A packet-approved approach with confirmed assumptions needs no repeat approval; routine milestones do not trigger review. Codex may request a checkpoint before affected implementation. The tool accepts exact sessionId/runId, category (approach/public_interface/plan_change/risk), proposal and rationale (each 1–768 characters, nonblank, combined at most 997). The Web plan-review detail contains both proposal and rationale within the 1024-character MCP envelope; the question text alone is not visibly rendered by that card. The category is a worker claim; the Host does not infer semantic change boundaries. Delegated runs do not require engineering checkpoints.

The Host awaits the native userQuestions channel with a stable `dsh-review:` question ID derived from runId/callId. The existing MCP question path immediately exposes it as QUESTION_REQUIRED with its native rpcId; dsh_answer_question answers the live question within the same run. Only a single exact Approve answer without custom conditions returns approved=true. Revise, custom feedback, cancellation, stale identities, channel failure or abort never approve. Codex decides in-scope engineering corrections; scope/acceptance changes, unresolved material architectural choices and sensitive/external effects retain the human boundary.

New reviewed admissions require `executionReviewContract=execution-lease-v1`. Host-private durable state owns execution authority; worker call/result messages and native question answers cannot create grants. Initial known reads are allowed, but the first effect waits in the native pre-execute pipeline until Codex issues `dsh_execution_control`. An omitted review therefore cannot silently begin implementation. Read-only tasks cannot acquire effect authority. An attempted operation outside a live grant closes the grant and waits for revision instead of repeatedly invoking a provider.

`status` returns exact session/run identity, monotonic generation, phase, paths, opaqueTools, expiry, remaining effects, waiting/active call counts and any independent review declaration. Mutations require `expectedGeneration`; ambiguous responses must be reconciled with status, not replayed. `grant` names a phase and bounded capabilities, for at most ten minutes and 64 effects (defaults ten minutes / 32). `renew` preserves the live phase/capabilities; expired or restarted leases require a new grant. `pause` revokes new effects, including calls delayed inside native approval; `revise` closes authority and returns held effects once with a denial so Root can investigate the correction. The final monotonic native guard rechecks the reserved generation after asynchronous policy/approval. Allowances are persisted before an effect is admitted, conservatively consumed on cancellation/crash, and shared across Root and its children. Child task markers cannot detach a child from its Host parent authority; effectful children must retain the canonical Root cwd.

Submitting `supervisor_review` withdraws the previous phase grant before asking the native question. Codex reads the proposal, reads current generation, grants the bounded phase, then answers the exact native question. Only a matching grant and exact Approve produce approved=true; revision/cancellation revokes a matching prematurely-issued grant. Native Web approval alone cannot grant execution. Paused/expired/restarted runs wait before the next provider request without polling a model. An in-flight model request or effect is not undone. `activeEffects=0` means no tracked calls, not that background processes or external effects are quiescent.

Known file tools receive relative-path, scope, symlink, hardlink and file-type preflight checks; the native workspace sandbox remains the backend boundary. This preflight is not a race-free narrow filesystem sandbox. Only explicitly audited opaque tool names can be granted, and only with original full-cwd scope plus an acknowledgement of unconfined indirect effects. The backend does not exclude network/hooks; neither the acknowledgement nor a phase grant expands user permissions. Narrow-scope shell execution is unsupported and must not be worked around by widening scope silently.

`dsh_wait`, recovery and durable run discovery expose Host execution state independently of worker summaries. Closed authority is a supervisor boundary checked at most every 30 seconds during an active wait. Ordinary activity retains the five-minute cadence. Codex performs an evidence-based continue/inspect/correct judgment before renewal; no automatic Codex relaunch or proof of its reasoning is claimed. After independently checking completed work, Codex uses `record_review` with verdict, compact evidence and exact asOfSeq. Host requires the current settled completed run tree at that boundary and records `SUPERVISOR_DECLARATION` separately from worker acceptance claims.

New reviewed tasks require Standard/native presentation. PTC remains rejected before provider setup because its outer code runtime cannot wait indefinitely. Historical receipts and old call/result review gates retain their original interpretation; a continuation receives fresh execution authority, not a restored grant. The `execution-lease-v1` capability and build identity reject mismatched installations. The build identity covers Host, MCP and packaged skill sources, but cannot prove which skill a running Codex turn already loaded. No additional model call estimates or renews authority.

## Reviewed evidence and independent sampling

A reviewed run may pin up to 16 `reviewWatchPaths` (workspace-relative file/directory prefixes) in its packet and receipt. They participate in request idempotency and are inherited by recovery continuations; they grant no write authority.

Worker progress may carry bounded `reviewEvidence`: up to 8 criterion claims (`workstreamId`, zero-based `doneWhenIndex`, `status=met|unmet|unknown`, evidence text), 3 plan changes, 5 negative evidence items, and 8 evidence paths. Text/path entries are capped at 256 characters. All lists and per-list `omitted` counts are required when this object is supplied. Old progress remains readable. The Host rejects malformed/oversized objects; MCP defensively marks malformed or unknown criterion references without discarding an otherwise valid risk signal. Evidence paths are unverified locators, not artifacts or scope exemptions. Compression retains unresolved failures and uncertainty before positive narrative.

Reviewed waits expose `reviewSignals` from the existing root-session Host event fold: watched-path changes, negative/pending verification, worker milestones, and missing/incomplete/invalid summary or partial activity coverage. Path matching happens before normal activity preview truncation; unknown shell effects and truncated Host snapshots remain coverage gaps. These are bounded review hints at the normal cadence, not new early-wakeup signals or a semantic success detector. `summaryCoverage=PROVIDED` means supplied claims, never verified truth.

The Codex skill independently samples actual critical files at material milestones, watched changes or evidence gaps. It compares distinct cadence windows for two observations lacking independently checkable progress; that semantic comparison lives in Codex context and is not a Host timeout/failure counter. Reconnect without that context triggers a fresh check. Codex may direct corrections inside the original task authority while Root owns implementation; scope/acceptance changes and sensitive or external effects remain human decisions. Final independent review is still required, and no extra model is called for compression, planning or evidence folding.

### Reviewed completion evidence gate

New reviewed admissions pin `terminalReviewContract=criteria-v1` in the durable packet and echo the admitted value in the receipt. Delegated runs do not set it. Retries return the original admitted contract, and recovery continuations inherit the parent value; historical runs without the field retain their previous completion rules. An upgrade never retrospectively invalidates a historical handoff.

For this contract, a completed `supervisor_handoff` must supply `finalReview` using the reviewEvidence shape with a larger bound of 40 criterion entries (5 workstreams × 8 doneWhen). Every doneWhen must appear exactly once, with a matching workstream/index, status met and nonblank evidence. A final table needs at least one workspace-relative evidence locator, zero omitted counts and no unresolved negativeEvidence. Plan-change, negative-evidence, locator and text bounds remain the same as progress evidence. Supporting admitted artifacts can hold detail but do not replace the table. Resolved failure history belongs in verification summaries or admitted reports rather than disappearing from the evidence trail.

The Host validates before artifact admission and before concluding the turn. An incomplete/invalid table rejects that tool call; blocked and failed exits remain available without a complete table. MCP independently rejects a malformed accepted completed result as FAILED/PROTOCOL_ERROR at terminal-review-evidence. A valid final table is returned as bounded worker evidence alongside REVIEW_TERMINAL; it cannot attest that Codex inspected files or independently reran verification. Existing identity, artifact confinement, Git scope, turn-end and child-convergence checks still apply.
