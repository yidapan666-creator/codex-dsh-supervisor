...
Standard and PTC durable call/result events carry the gate. The native monotonic tool guard rejects new execution, spawning and completed handoff for the Root and affiliated children until a corresponding successful approved result. Known reads, Root review/progress/failure reporting and non-completed handoff remain available. New reviewed tasks require Standard/native presentation. MCP rejects reviewed+code before model selection, and Host admission independently rejects a code preset or an effective run_code presentation before queueing. Historical receipt reconciliation remains available, but a pending incompatible admission is not rearmed after restart. The review tool itself rejects PTC, so a legacy code run cannot repeatedly open timed questions. Historical PTC gate parsing and restricted read wrappers remain available for defensive replay; they do not imply supported PTC review. Root must quiesce relevant child work before requesting review. This cannot undo an already-running effect or automatically detect a major change which was never submitted.

Rejected, cancelled and unanswered reviews survive replay and Host continuation through the recorded parentRunId chain; recovery requires a fresh native question. Generic steer and observation cursors do not release the gate. The capability `blocking-supervisor-review-v1` and the lockstep plugin build ID fail closed on an old Host plugin. No new queue, approval database, periodic model call or review artifact is introduced. At every distinct five-minute cadence Codex must choose continue, inspect or correct against doneWhen; this engineering judgment is a skill obligation, not a Host attestation of Codex behavior.
