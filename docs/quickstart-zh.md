...
Standard/PTC 决定 DSH 怎样执行；监管力度决定 Codex 怎样介入过程和验收，两者不是同一个开关。目前审核监管仅支持 Standard；PTC 可使用轻监管，因为 PTC 的执行时限会取消长时间未答复的审批。入口会明确拒绝不兼容组合，不会自动降级或重复派发：

| 监管力度 | 适合什么任务 | Codex 行为 |
| --- | --- | --- |
| 轻监管 `delegated` | 边界明确、低风险、通常是只读或原子任务 | 五分钟聚合观察；终态仍核对 Host 范围、活动和结构化验证证据 |
| 审核监管 `reviewed` | 写代码、跨模块、公共接口、数据、安全或恢复链路 | 派发前明确技术方向；重大方案实施前等待 Codex 审批或纠偏；每五分钟判断方向；高风险立即介入，完成后独立验收 |

审核监管中的方案检查点会真正等待答复；普通进度摘要或 steer 不算批准。Codex 可直接处理已授权范围内的工程修正；扩大范围、安全敏感或外部副作用仍由你决定。检查点等待期间，Host 拦住 Root 和相关子 Agent 的新执行动作，但不能撤回已经在执行的动作。识别重大方案仍依赖 DSH/Codex 的工程判断，不能保证自动发现所有偏差。

writer 默认使用审核监管，read-only 默认使用轻监管。你可以明确指定，档位会写入 durable run，Codex/MCP 断连恢复后不会改变。

## 最简单的调用方式

在 Codex 中直接说：

> 用 DSH 在 `/绝对路径/项目目录` 完成登录接口的超时修复。使用审核监管：先复现，再修改并跑相关测试，完成后由 Codex 独立验收。允许最多 3 个直接子 agent；任务 token 上限 60000。每五分钟给我一次聚合进度，打开 DSH Web 让我能看到会话。
...
1. 记录 Git 基线，决定新建或复用 Root，并分别选择 Standard/PTC 执行模式和轻监管/审核监管。
...
5. 只在严格完成条件成立后汇报协议完成；审核监管还要由 Codex 检查实际改动、独立重跑关键验证，再汇报验收结果。
